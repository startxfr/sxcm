...
oc wait po -l name=$SERVER_NAME --for=condition=Ready --timeout=500ms -n $PROJECT &>/dev/null
if [[ $? == "0" ]]
then
    ...
    sleep ${DELAY}
    oc wait po -l name=$SERVER_NAME --for=condition=Ready --timeout=${DELAY}0ms -n $PROJECT &>/dev/null
    if [[ $? == "0" ]]
    then
    ...
    oc adm policy add-scc-to-user anyuid -z argocd-redis-ha -n $PROJECT &>/dev/null
fi
